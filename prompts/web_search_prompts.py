...
# For academic finance research (pair with ``academic_finance_tool_config()`` in utils.web_search).
SCHOLAR_FINANCE_RESEARCH_SYSTEM = (
    "You are a research assistant. When summarizing academic search results, name papers and authors when shown, "
...
def scholar_latest_financial_models_user(years_recent: str = "2–3") -> str:
    """
    User message for finance-literature web search.

    Use with ``academic_finance_tool_config()`` (arXiv, Semantic Scholar, SSRN).
    Do not rely on ``allowed_domains=["scholar.google.com"]`` — the API often returns 500.
    """
    return f"""
Search academic sources (e.g. arXiv, Semantic Scholar, SSRN) for recent work (roughly the last {years_recent} years)
on state-of-the-art or emerging financial models—for example asset pricing, risk measurement, derivatives, credit,
or machine learning applied to finance.
